...
bool CConstantBuffer::Init(int Size, int Register, int ShaderBufferType)
{
	...
	// 상수버퍼는 정보갱신이 많기 때문에 Dynamic 으로 만들어준다.
	// 예를 들어서 플레이어 이동 할 때마다 갱신이 일어나기 때문에
...
{
	// Buffer 안에 있는 데이터를 저장하기 위한 주소를 가져온다.
	D3D11_MAPPED_SUBRESOURCE Map = {};
	// Map : CPU가 GPU를 사용하기 위해서 잠금 처리하는 것이다.
	...
	// Map을 사용했으면 반드시 Unmap을 해주어야 한다.
	CDevice::GetInst()->GetContext()->Unmap(mBuffer, 0);

	// 위에서는 GPU 메모리 공간에 우리가 계산한 데이터를 넣어준 것이다.
